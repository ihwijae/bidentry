...
2. `automation-engine/src/core/orchestrator.js`는 작업 수명주기를 관리합니다.
   - `web/playwright.js`가 자동화 전용 Chrome 프로필을 띄워 사용자의 기존 브라우저와 충돌하지 않게 하고, 필요 시 레지스트리 정책(`InsecurePrivateNetworkRequestsAllowedForUrls`)을 설정한 뒤 로컬 네트워크 권한 팝업을 UI 자동화로 허용하고 사이트별 로그인 훅(`loginKepco`, `loginMnd`)을 호출합니다.
