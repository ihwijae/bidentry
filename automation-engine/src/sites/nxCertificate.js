...
async function handleNxCertificate(siteLabel, page, emit, cert = {}, extra = {}) {
  ...
  const confirmDelay = fastMode ? 140 : 300;
  const identityDelay = fastMode ? 110 : 250;
  const passwordDelay = fastMode ? 90 : 200;
  const postCloseDelay = fastMode ? 180 : 500;
  const retryBaseDelay = fastMode ? 60 : 120;
  const retryStepDelay = fastMode ? 40 : 80;
  const selectionAttempts = fastMode ? 3 : 4;
